...
def construct_model(images,
    ...
    gen_states, gen_images, gen_masks, gen_poses, gen_retina = [], [], [], [], []
    true_retina = []
    summaries = []
    ...
    for image, himage, action, state in zip(images[:-1],highres_images[:-1], actions[:-1], states[:-1]):
        ...
        with slim.arg_scope(
            ...
            if feedself and done_warm_start:
                ...
                prev_retina = scheduled_sample(true_retinas[-1], gen_retinas[-1], batch_size, num_ground_truth)
                prev_retina = tf.reshape(prev_retina, [conf['batch_size'], 64, 64, 3])
...
def get_retina(himages, pix_distrib, poses, t):
