...
class Occlusion_Model(object):
    def __init__(self,
        ...
        self.batch_size, self.img_height, self.img_width, self.color_channels = [int(i) for i in image[0].get_shape()[0:4]]
    ...
    def build(self):
        ...
        with slim.arg_scope(
            ...
            assembled_image = tf.mul(backgd_mask, self.image[0]) + tf.mul(composed_mask, pre_assembly)
    ...
    def decompose_firstimage(self, enc6):
        ...
        for mask in mask_list:
            image_partlist.append(tf.mul(self.image[0], mask))
