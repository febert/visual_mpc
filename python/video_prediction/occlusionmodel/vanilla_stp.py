...
def construct_model(images,
    ...
    for image, action in zip(images[:-1], actions[:-1]):
        ...
        with slim.arg_scope(
            ...
            # allways transform_from_firstimage
            prev_image = images[1]
...
def stp_transformation_mask(conf, first_image, prev_mask_list, stp_input, num_masks, reuse=None):
    """Apply spatial transformer predictor (STP) to previous image.
    ...
    identity_params = tf.tile(identity_params, [32, 1])

    transformed_parts = []
    transformed_masks = []
    transforms = []

    if 'movement_factor' in conf:
        movement_factor = conf['movement_factor']
    else:
        movement_factor = 1.
    ...
    for i in range(num_masks):
        ...
        transformed_part = tf.reshape(transformed_part, [32, 64, 64, 3])
        transformed_parts.append(transformed_part)

        transf_mask = transformer(prev_mask_list[i], params, outsize)
        transf_mask = tf.reshape(transf_mask, [32, 64, 64, 1])
