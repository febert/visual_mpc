...
from lsdc.algorithm.policy.tf_policy import TfPolicy
from lsdc.algorithm.policy_opt.policy_opt_tf import PolicyOptTf
from lsdc.algorithm.policy_opt.config import POLICY_OPT_TF
from lsdc.algorithm.policy_opt.tf_model_example import euclidean_loss_layer, \
...
def test_policy_save():
    ...
    deg_action = 7
    policy_opt = PolicyOptTf(hyper_params, deg_obs, deg_action)
    check_path = gps_path + '/lsdc/algorithm/policy_opt/tf_checkpoint/policy_checkpoint'
    policy_opt.policy.pickle_policy(deg_obs, deg_action, check_path)


def test_policy_load():
    tf_map = POLICY_OPT_TF['network_model']
    check_path = gps_path + '/lsdc/algorithm/policy_opt/tf_checkpoint/policy_checkpoint/_pol'
