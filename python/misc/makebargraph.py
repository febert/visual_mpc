...
def long_dist_task():
    ...
    # order
    dist_means_novel = [17.7, 18.3, 25.5, 30.2]  # Mean Data
    std_dev_means_novel = [9.7, 9.5, 11.0, 3.6]

    dist_means_seen = [8.93, 12.8, 26.2, 29.3]  # Mean Data
    std_dev_means_seen = [7.9, 6.6, 12.7, 3.2]
    ...
    plt.bar(ind, dist_means_novel, width, color='b', align='center', yerr=yerr_novel, ecolor='k', label='novel objects moved',alpha = .6)

    plt.bar(ind - width, dist_means_seen, width, color='g', align='center', yerr=yerr_seen, ecolor='k',alpha = .6,
            label='seen objects moved stationary')
    ...
    # plt.show()
    plt.savefig('/home/guser/frederik/doc_video/results_longdistance.png')
...
def mult_task():
    ...
    plt.bar(ind - width*3, dist_means_novel_move, width, color='b', align='center', yerr=yerr_novel_move, ecolor='k', label='novel objects moved', alpha = .6)
    plt.bar(ind-width*2, dist_means_seen_move, width, color='g', align='center', yerr=yerr_seen_move, ecolor='k', label='seen objects moved', alpha = .6)
    ...
    plt.legend(loc="upper right")
    # plt.show()
    plt.savefig('/home/guser/frederik/doc_video/results_mult_obj.png')
