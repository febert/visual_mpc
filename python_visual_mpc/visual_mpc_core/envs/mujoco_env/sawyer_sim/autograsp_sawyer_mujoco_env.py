...
class AutograspSawyerMujocoEnv(BaseSawyerMujocoEnv):
    ...
    def _init_dynamics(self):
        ...
        if self._params.no_motion_goal:
            self._last_objects = np.zeros((self._params.num_objects, 3))
            touch_offset = 0
            if self.finger_sensors:
                touch_offset = 2
            for i in range(self._params.num_objects):
    ...
    def _post_step(self):
        if not self._params.no_motion_goal:
            ...
            last_objects = np.zeros((self._params.num_objects, 3))
            touch_offset = 0
            if self.finger_sensors:
                touch_offset = 2
