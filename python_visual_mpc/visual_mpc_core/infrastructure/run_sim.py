...
class Sim(object):
    ...
    def __init__(self, config, gpu_id=0, ngpu=1, logger=None):
        ...
        if 'netconf' in config['policy']:
            ...
            elif 'openloop_conf' in config['policy']:
                self.openloop_policy = config['policy']['openloop_setup'](config['policy']['openloop_conf'])
                self.policy = config['policy']['type'](config['agent'], config['policy'], self.predictor, self.openloop_policy)
            else:
    ...
    def _init_policy(self):
        if 'netconf' in self.policyparams:
            ...
            elif 'openloop_conf' in self.policyparams:
                self.policy = self.policyparams['type'](self.agent._hyperparams, self.policyparams, self.predictor, self.openloop_policy)
            else:
                self.policy = self.policyparams['type'](self.agent._hyperparams,
