    ...
    def save_data(self, traj, itr):
        ...
        if 'save_raw_images' in self._hyperparams:
            ...
            for t in range(traj.T):
                for icam in range(self.agent.ncam):
                    image_name = self.image_folder+ "/im{}_cam{}.png".format(t, icam)
                    cv2.imwrite(image_name, traj._sample_images[t, icam][:,:,::-1], [cv2.IMWRITE_PNG_STRATEGY_DEFAULT, 1])
