...
# import pdb
# pdb.set_trace()
# from utils.timer import Timer  ## orig
from timer import Timer  ## new

import numpy as np
import cv2
from fast_rcnn.nms_wrapper import nms
import pickle
# from utils.blob import im_list_to_blob  ## orig
...
def im_proposal_tensorflow(sess_tuple, im):
    blobs, im_scales = _get_blobs(im, rois=None)

    sess, ph_data, ph_im_info, out_rois = sess_tuple
    ...
    rois = sess.run(out_rois, {ph_data: blobs['data'], ph_im_info: blobs['im_info']})
...
def im_detect_tensorflow(sess_tuple, im):
    ...
    rois, scores, box_deltas = sess.run((out_rois, out_cls_prob, out_bbox_pred),
        {ph_data: blobs['data'], ph_im_info: blobs['im_info']})
...
def im_detect_caffe(net, im, boxes=None):
    """Detect object classes in an image given object proposals.
